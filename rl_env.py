...
def evaluate_agent(model, data_load, device, verbose=True):
    ...
    model.set_patches(torch.tensor(patches.float()))
...
class OurViTEnv(gym.Env):
    def __init__(self, ViTnet, n_patch, optimizer, loss_weight, time_weight, device, n_patch_selected = 1, seed = None, verbose=False, total_epochs=20):
        super().__init__()
